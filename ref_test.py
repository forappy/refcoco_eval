...
class RefDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        norm_bbox = self.scale_and_adjust_bbox(bbox, (w, h))
         # 将每个边界框坐标截取到小数点后三位
    ...
    def scale_and_adjust_bbox(self, bbox, original_size, target_side=560):
        x, y, width, height = bbox
        orig_w, orig_h = original_size

        if orig_w > orig_h:
            scale = target_side / orig_w
        else:
            scale = target_side / orig_h
        ...
        scaled_bbox = [scaled_x, scaled_y, scaled_width, scaled_height]

        # New dimensions after initial resize
        new_width = orig_w * scale
        new_height = orig_h * scale
        
        # Padding calculation
        if new_width < target_side:
            pad_x = (target_side - new_width) / 2
            pad_y = 0
        else:
            pad_x = 0
            pad_y = (target_side - new_height) / 2

        # Adjust bbox for padding
        adjusted_bbox = [
            scaled_bbox[0] + pad_x,
            scaled_bbox[1] + pad_y,
            scaled_bbox[0] + scaled_bbox[2] + pad_x,
            scaled_bbox[1] + scaled_bbox[3] + pad_y
        ]

        # Normalize with respect to target dimensions
        final_bbox = [
            adjusted_bbox[0] / target_side,
            adjusted_bbox[1] / target_side,
            adjusted_bbox[2] / target_side,
            adjusted_bbox[3] / target_side
        ]
